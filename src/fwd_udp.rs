...
pub fn udp_ipv6_init(interface_name:&str)->std::net::UdpSocket{
  ...
  get_udpsock_with_mcastv6_opts(&socket_addr_v6,&PINECONE_UDP_MCAST_ADDR,ifindex,interface_name).expect("Failed to set multicast options")
...
fn get_udpsock_with_mcastv6_opts(addr:&SocketAddrV6,multiaddr:&Ipv6Addr,if_index:u32,if_name:&str)->Result<std::net::UdpSocket, Box<dyn Error>> {
    ...
    socket.bind_device(Some(if_name.as_bytes())).expect("bind device err");
  // socket.set_only_v6(true).unwrap_or_else(|e|{println!("error : {}",e)});
    socket.join_multicast_v6(multiaddr, if_index).expect("join multicast v6 error");
    socket.set_multicast_if_v6(if_index).expect("set multicast interface v6");

    socket.set_multicast_loop_v6(false).expect("set mcast loop v6 err");
